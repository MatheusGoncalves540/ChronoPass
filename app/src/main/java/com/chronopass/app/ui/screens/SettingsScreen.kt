...
fun SettingsScreen(vm: ChronoViewModel, nav: NavController) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Configurações") }) }) { pad ->
        Column(
                ...
                Button(
                        onClick = {
                            ...
                            if (la != null && lo != null && r != null) {
                                vm.saveStore(name.trim(), la, lo, r)
                                msg = "Loja salva."
            ...
            Button(
                    enabled = newPw.length >= 4,
                    onClick = {
                        vm.setAdminPassword(newPw)
                        newPw = ""
            ...
            Text(
                    "Funcionários e marcações excluídos ficam aqui. Limpar apaga em definitivo.",
            ...
            Text(
                    "Ponto eletrônico simples: registro de entrada/saída por funcionário, com localização, foto e relatórios em PDF.",
                    style = MaterialTheme.typography.bodySmall
            )
    ...
    manualUpdateInfo?.let { info ->
        UpdateAvailableDialog(
                ...
                onInstallLaunched = { manualUpdateInfo = null }
        )
    }
    ...
    if (confirmTrash) {
        AlertDialog(
                ...
                text = {
                    Text(
                            "Apagar em definitivo $trashCount item(ns)? Esta ação não pode ser desfeita."
                    )
                },
                confirmButton = {
                    TextButton(
                            onClick = {
                                ...
                                vm.emptyTrash {
                                    ...
                                    msg = "Lixeira esvaziada."
                                }
                            }
                    ) { Text("APAGAR TUDO") }
                },
                dismissButton = {
                    TextButton(onClick = { confirmTrash = false }) { Text("Cancelar") }
                }
        )
    }
}
