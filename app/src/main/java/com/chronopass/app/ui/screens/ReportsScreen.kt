...
fun ReportsScreen(vm: ChronoViewModel, nav: NavController) {
    ...
    val punches by
            remember(from, to) { vm.repo.punchesBetween(from, to) }
    ...
    fun exportPdf(e: Employee) {
        scope.launch {
            ...
            withContext(Dispatchers.IO) {
                PdfExport.write(file, e.name, periodLabel, rows, logo, photo)
            }
                        ...
                        scope.launch {
                            withContext(Dispatchers.IO) {
                                context.contentResolver.openInputStream(uri)?.use {
                                    BackupManager.import(context, it)
                                }
                            }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Relatórios e backup") }) }) { pad ->
        Column(
            ...
            Row(Modifier.horizontalScroll(rememberScrollState())) {
                ReportPeriod.entries.forEach { p ->
                    FilterChip(
                            selected = period == p,
                            onClick = {
                                if (p == ReportPeriod.CUSTOM) datePicker = true else period = p
                            },
                            label = { Text(p.label) }
                    )
                        ...
                        if (shown.isEmpty()) {
                            status = "Nenhuma marcação no período."
                                ...
                                scope.launch {
                                    ...
                                    withContext(Dispatchers.IO) {
                                        CsvExport.write(file, rows, names)
                                    }
                                    share(file, "text/csv")
                                }
                    },
            ...
            Button(
                    onClick = {
                        selectedEmployee.let { if (it != null) exportPdf(it) else pdfPicker = true }
                    },
                    Modifier.fillMaxWidth()
            ) {
                Text(
                        selectedEmployee?.let { "GERAR PDF — ${it.name}" }
                                ?: "GERAR PDF (por funcionário)"
                )
            }
            ...
            Button(
                    onClick = { backupSaver.launch("chronopass-backup-${today()}.zip") },
                    Modifier.fillMaxWidth()
            ) { Text("EXPORTAR BACKUP") }
            OutlinedButton(
                    onClick = { backupRestorer.launch(arrayOf("application/zip")) },
                    Modifier.fillMaxWidth()
                    ...
                    Row(
                        ...
                        TextButton(
                                ...
                                onClick = {
                                    ...
                                    customRange =
                                            TimeUtil.startOfDay(TimeUtil.fromPickerUtc(s)) to
